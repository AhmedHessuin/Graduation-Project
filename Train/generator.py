...
class BatchGenerator(Sequence):
    ...
    def __len__(self):
        return int(np.ceil(float(len(self.instances))/self.batch_size)) #return the ceil in numpy array format
    ...
    def __getitem__(self, idx):
        ...
        #===================================== remaining data size > batch size =============================================#
        l_bound = idx*self.batch_size
        r_bound = (idx+1)*self.batch_size
        # batch size = 10 for example
        #case 1
        #left bound = 0 * 10 =0
        #right bound = 1 * 10 = 10
        ...
        for train_instance in self.instances[l_bound:r_bound]:
            ...
            for obj in all_objs:
                ...
                shifted_box = BoundBox(0, 
                                       0,
                                       obj['xmax']-obj['xmin'],                                                
                                       obj['ymax']-obj['ymin'])    #creating a bb
                
                for i in range(len(self.anchors)):#finding the best anchor
                    anchor = self.anchors[i] # get anchor i
                    iou    = bbox_iou(shifted_box, anchor) # see the iou between the anchor and the bbox

                    if max_iou < iou: #if max_iou is  less than the iou
                        max_anchor = anchor#uodated the max anchor
                        max_index  = i #update the max index
                        max_iou    = iou  #update the max iou
                
                # determine the yolo to be responsible for this bounding box
                yolo = yolos[max_index//3] # the yolo is made of 3 layers what layer will be responsible for this bb
                grid_h, grid_w = yolo.shape[1:3]
                
                # determine the position of the bounding box on the grid
                center_x = .5*(obj['xmin'] + obj['xmax']) # in the center of the bb
                center_x = center_x / float(net_w) * grid_w # sigma(t_x) + c_x
                center_y = .5*(obj['ymin'] + obj['ymax']) # in the center of the bb
    ...
    def _aug_image(self, instance, net_h, net_w):
        ...
        dw = self.jitter * image_w; # adding noise
        dh = self.jitter * image_h; # adding noise
    ...
    def on_epoch_end(self):
        if self.shuffle: np.random.shuffle(self.instances) # shuffle data with every epoch
    ...
    def load_image(self, i):
        return cv2.imread(self.instances[i]['filename'])      # load the image with open cv